...
import { Button } from "../../src/oneui/button";

describe("Samsung One UI Button", () => {
  describe("Rendering", () => {
  ...
  describe("Samsung One UI Styling", () => {
    it("should have semibold text styling", () => {
      render(<Button>OneUI Button</Button>);
      const button = screen.getByRole("button");
      expect(button).toHaveClass("font-semibold");
    });

    it("should have 16px border radius (notably rounded)", () => {
      render(<Button>OneUI Button</Button>);
      const button = screen.getByRole("button");
      expect(button).toHaveClass("rounded-2xl");
    });

    it("should have Samsung One UI transitions with smooth easing", () => {
      render(<Button>OneUI Button</Button>);
      const button = screen.getByRole("button");
      expect(button).toHaveClass("transition-all");
      expect(button).toHaveClass("duration-250");
      expect(button.className).toContain("ease-[cubic-bezier(0.33,0,0.2,1)]");
    });

    it("should have prominent Samsung One UI shadows", () => {
      render(<Button variant="default">OneUI Button</Button>);
      const button = screen.getByRole("button");
      expect(button.className).toContain("shadow-[0_4px_12px_rgba(0,0,0,0.1)]");
    });
  });
